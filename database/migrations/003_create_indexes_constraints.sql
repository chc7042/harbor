...
-- 체크 제약조건 추가
-- 빌드 번호는 양수여야 함
ALTER TABLE deployments ADD CONSTRAINT check_build_number_positive
CHECK (build_number > 0);

-- 파일 크기는 0 이상이어야 함
ALTER TABLE artifacts ADD CONSTRAINT check_file_size_non_negative
CHECK (file_size >= 0);

-- 빌드 지속시간은 0 이상이어야 함 (NULL 허용)
ALTER TABLE deployments ADD CONSTRAINT check_build_duration_non_negative
CHECK (build_duration IS NULL OR build_duration >= 0);

-- Git 커밋 해시 형식 검증 (40자 16진수)
ALTER TABLE deployments ADD CONSTRAINT check_git_commit_format
CHECK (git_commit IS NULL OR git_commit ~ '^[a-f0-9]{40}$');
...
ALTER TABLE deployments ADD CONSTRAINT fk_deployments_project
FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE;

ALTER TABLE artifacts DROP CONSTRAINT IF EXISTS artifacts_deployment_id_fkey;
ALTER TABLE artifacts ADD CONSTRAINT fk_artifacts_deployment
FOREIGN KEY (deployment_id) REFERENCES deployments(id) ON DELETE CASCADE;

ALTER TABLE deployment_parameters DROP CONSTRAINT IF EXISTS deployment_parameters_deployment_id_fkey;
ALTER TABLE deployment_parameters ADD CONSTRAINT fk_deployment_parameters_deployment
FOREIGN KEY (deployment_id) REFERENCES deployments(id) ON DELETE CASCADE;
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_artifacts_large_files
